...
Two-tool Bash package wrapping `gh` + `git` (no k8-lib): **`bin/make-repo`** (~1560 lines) create/edit GitHub repos; **`bin/fork-repo`** (~436 lines) fork + remote rewiring. No persistence layer — data artifacts (env vars, flag grammar, git files) documented in `PROJ-SCHEMA.md`.
